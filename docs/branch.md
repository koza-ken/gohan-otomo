...
　---------------------------
  ...
  - Postモデルにsearchスコープメソッド追加
  - title・descriptionでの部分一致検索実装
  - 検索フォームUI実装（投稿一覧上部）
  - 検索結果の「該当なし」状態ハンドリング
  - SQLインジェクション対策確認
  ...
  - kaminari gem追加・設定
  - ページネーションビューの実装・カスタマイズ
  - ページサイズ設定（12投稿/ページ等）
  - ページネーションとフィルター/検索の連携
  - SEO対応（rel=prev/next）

  📋 Task 5: フィルター機能の実装

  - 投稿タイプ別フィルター実装
    - 全投稿・画像付き投稿・外部URL画像投稿
  - フィルターUI実装（タブまたはボタン）
  - アクティブフィルターの視覚的表示
  - フィルターとページネーション連携
  ...
  - Request spec: 検索・ソート・ページネーションテスト
  - System spec: UI操作の統合テスト実装
  - Model spec: searchスコープメソッドテスト
  - フィルター機能のテスト
  - エッジケース対応テスト
