...
- **image_processing gem有効化**: Gemfileで `gem "image_processing", "~> 1.2"` を有効化
- **Active Storageインストール**: `rails active_storage:install` でマイグレーション作成
- **データベース準備**: 3つのテーブル作成（active_storage_blobs, attachments, variant_records）
- **技術基盤完成**: Rails 7標準の画像アップロード基盤構築

#### ✅ Task 2: 画像アップロード機能の実装 
**Task 2-1: Postモデルの拡張**
```ruby
# app/models/post.rb
has_one_attached :image  # Active Storage関連の追加
```

**Task 2-2: PostsControllerの更新**
```ruby
# Strong Parametersに:imageを追加
def post_params
  params.require(:post).permit(:title, :description, :link, :image_url, :image)
end
```

**学習ポイント**: Active Storageは既存テーブル変更不要、関連テーブルで管理

#### ✅ Task 3: 投稿フォームの画像アップロード対応
**Task 3-1 & 3-2: HTMLフォーム拡張**
- new.html.erb と edit.html.erb にfile_fieldを追加
- `accept="image/*"` でブラウザ側ファイル形式制限

**Task 3-3: Stimulus画像プレビュー機能**
```javascript
...
**Task 3-4: ユーザビリティ向上**
- ファイル形式・サイズ制限の事前案内表示
- "JPEG、PNG、GIF、WebP形式で10MB以下のファイル" の表示
